...
router.post('/', requirePMAdmin, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO request_categories (name, description, icon, sort_order) VALUES (?, ?, ?, (SELECT COALESCE(MAX(sort_order),0)+1 FROM request_categories))')
    .run(name.trim(), description?.trim() || null, icon?.trim() || '🏷');
...
router.patch('/:id', requirePMAdmin, (req, res) => {
  ...
  db.prepare('UPDATE request_categories SET name=?, description=?, icon=?, active=? WHERE id=?')
    .run(
      name?.trim() || cat.name,
      description !== undefined ? (description?.trim() || null) : cat.description,
      icon !== undefined ? (icon?.trim() || '🏷') : cat.icon,
