...
route('new-booking', async ({ date, amenityId } = {}) => {
      ...
      <div class="card-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="date" class="form-input" id="bk-date" value="${date || ''}" min="${new Date().toLocaleDateString('en-CA')}" onchange="loadTimeSlots()">
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <div class="time-slots" id="start-slots"><div style="color:var(--gray-400);font-size:.85rem">Select a date first</div></div>
        </div>
        <div class="form-group" id="end-time-group" style="display:none">
          <label class="form-label required">End Time</label>
          <div class="time-slots" id="end-slots"></div>
        </div>
  ...
  window._selectedStart = null;
  window._selectedEnd   = null;
  window._resourceQtys  = {};
  window._bookingAmenities = amenities;
  loadAmenityResources();
  if (date) loadTimeSlots();
});
...
async function loadTimeSlots() {
  ...
  const slotsEl = document.getElementById('start-slots');
  slotsEl.innerHTML = '<div class="spinner"></div>';
  window._selectedStart = null; window._selectedEnd = null;
  document.getElementById('end-time-group').style.display = 'none';

  const data = await apiFetch('GET', `/api/bookings/calendar?amenity_id=${amenityId}&year=${date.slice(0,4)}&month=${parseInt(date.slice(5,7))}`).catch(()=>({bookings:[],blackouts:[]}));

  const dateBookings = data.bookings.filter(b => new Date(b.start_time).toLocaleDateString('en-CA') === date && b.status === 'confirmed');
  const dateBlackouts = data.blackouts.filter(bl => {
    const s = new Date(bl.start_time).toLocaleDateString('en-CA');
    const e = new Date(bl.end_time).toLocaleDateString('en-CA');
    return s <= date && date <= e;
  });

  const HOURS = Array.from({length:14}, (_,i) => i+7); // 7am to 8pm
  function isBookedHour(h) {
  ...
  function isBlockedHour(h) {
    return dateBlackouts.some(bl => {
      const blStart = new Date(bl.start_time).getHours();
      const blEnd   = new Date(bl.end_time).getHours();
      return h >= blStart && h < blEnd;
    });
  }

  slotsEl.innerHTML = HOURS.map(h => {
    const label = fmt12(h, 0);
    const booked = isBookedHour(h);
    const blocked = isBlockedHour(h);
    const cls = booked ? 'booked' : blocked ? 'blocked' : '';
    const onclick = (!booked && !blocked) ? `onclick="selectStartTime(${h})"` : '';
    return `<div class="time-slot ${cls}" id="start-${h}" ${onclick}>${label}${blocked?' (Blocked)':''}</div>`;
  }).join('');

  window._dayBookings = dateBookings;
  window._dayBlackouts = dateBlackouts;
}

function selectStartTime(h) {
  document.querySelectorAll('#start-slots .time-slot').forEach(el => el.classList.remove('selected'));
  document.getElementById('start-' + h)?.classList.add('selected');
  window._selectedStart = h;
  window._selectedEnd   = null;
  renderEndSlots(h);
  document.getElementById('end-time-group').style.display = '';
}

function renderEndSlots(startH) {
  const HOURS = Array.from({length:13}, (_,i) => startH + i + 1).filter(h => h <= 21);
  const endEl = document.getElementById('end-slots');
  endEl.innerHTML = HOURS.map(h => {
    const booked = (window._dayBookings||[]).some(b => {
      const bStart = new Date(b.start_time).getHours();
      const bEnd   = new Date(b.end_time).getHours();
      return h > bStart && h <= bEnd;
    });
    const blocked = (window._dayBlackouts||[]).some(bl => {
      const blStart = new Date(bl.start_time).getHours();
      const blEnd   = new Date(bl.end_time).getHours();
      return h > blStart && h <= blEnd;
    });
    const cls = booked ? 'booked' : blocked ? 'blocked' : '';
    const onclick = (!booked && !blocked) ? `onclick="selectEndTime(${h})"` : '';
    return `<div class="time-slot ${cls}" id="end-${h}" ${onclick}>${fmt12(h,0)}</div>`;
  }).join('');
}

function selectEndTime(h) {
  document.querySelectorAll('#end-slots .time-slot').forEach(el => el.classList.remove('selected'));
  document.getElementById('end-' + h)?.classList.add('selected');
  window._selectedEnd = h;
}
...
async function submitBooking() {
  ...
  const startH    = window._selectedStart;
  const endH      = window._selectedEnd;

  if (!date)     { toast('Please select a date', 'warning'); return; }
  if (!startH && startH !== 0) { toast('Please select a start time', 'warning'); return; }
  if (!endH)     { toast('Please select an end time', 'warning'); return; }
  ...
  const end   = new Date(`${date}T${String(endH).padStart(2,'0')}:00:00`);
