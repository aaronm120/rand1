...
router.get('/users', requirePMAdmin, (req, res) => {
  const users = db.prepare(`
    SELECT u.id, u.email, u.name, u.role, u.tenant_id, u.title, u.phone,
           u.directory_opt_out, u.active, u.created_at,
...
router.put('/users/:id', requirePMAdmin, (req, res) => {
  const { name, role, tenant_id, title, phone, active, password } = req.body;
  ...
  db.prepare(`UPDATE users SET name=?, role=?, tenant_id=?, title=?, phone=?, active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`)
    .run(name || user.name, role || user.role, tenant_id ?? user.tenant_id,
      title ?? user.title, phone ?? user.phone, active !== undefined ? (active ? 1 : 0) : user.active, req.params.id);
...
// Tenant Admin: manage users within their own tenant
